...
#include "mali_names.hpp"

#if defined(ANDROID) || defined(__ANDROID__)
/* We use _IOR_BAD/_IOW_BAD rather than _IOR/_IOW otherwise fails to compile
 * with NDK-BUILD because of _IOC_TYPECHECK is defined, not because the paramter
 * is invalid */
...
// mali_device_api_version
struct kbase_ioctl_version_check {
    uint16_t major;
    uint16_t minor;
};

// mali_context_set_creation_flags
...
#define KBASE_IOCTL_GET_GPUPROPS  \
    MALI_IOW(KBASE_IOCTL_TYPE, 3, \
             struct mali_userspace::kbase_ioctl_get_gpuprops)
#define KBASE_IOCTL_VERSION_CHECK_JM \
    _IOWR(KBASE_IOCTL_TYPE, 0, struct mali_userspace::kbase_ioctl_version_check)
#define KBASE_IOCTL_VERSION_CHECK_CSF \
    _IOWR(KBASE_IOCTL_TYPE, 52,       \
          struct mali_userspace::kbase_ioctl_version_check)
#define KBASE_IOCTL_SET_FLAGS \
    _IOW(KBASE_IOCTL_TYPE, 1, struct mali_userspace::kbase_ioctl_set_flags)
#define KBASE_IOCTL_HWCNT_READER_SETUP \
    _IOW(KBASE_IOCTL_TYPE, 8,          \
         struct mali_userspace::kbase_ioctl_hwcnt_reader_setup)
...
enum {
    ...
    KBASE_HWCNT_READER_GET_BUFFER = MALI_IOR(
            KBASE_HWCNT_READER, 0x20, struct kbase_hwcnt_reader_metadata),
    KBASE_HWCNT_READER_PUT_BUFFER = MALI_IOW(
            KBASE_HWCNT_READER, 0x21, struct kbase_hwcnt_reader_metadata),
...
enum {
    POLL_DESCRIPTOR_SIGNAL, /**< The index of the signal descriptor in poll fds
                               array. */
    POLL_DESCRIPTOR_HWCNT_READER, /**< The index of the hwcnt reader descriptor
                                     in poll fds array. */
...
static inline int mali_ioctl(int fd, T& arg) {
    auto* hdr = &arg.header;
    const int cmd = _IOC(_IOC_READ | _IOC_WRITE, LINUX_UK_BASE_MAGIC, hdr->id,
                         sizeof(T));
