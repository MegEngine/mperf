...
MaliHWInfo get_mali_hw_info(const char* path) {
...

    {
        ...
        mali_userspace::kbase_uk_hwcnt_reader_version_check_args
                version_check_args;
        version_check_args.header.id =
                mali_userspace::UKP_FUNC_ID_CHECK_VERSION_JM;  // NOLINT
        version_check_args.major = 10;
        version_check_args.minor = 2;

        if (mali_userspace::mali_ioctl(fd, version_check_args) != 0) {
            mali_userspace::kbase_ioctl_version_check _version_check_args = {0,
                                                                             0};
            if (ioctl(fd, KBASE_IOCTL_VERSION_CHECK_JM, &_version_check_args) <
                0) {
        ...
        if (!checked_version) {
            version_check_args.header.id =
                    mali_userspace::UKP_FUNC_ID_CHECK_VERSION_CSF;
            version_check_args.major = 1;
            version_check_args.minor = 4;

            if (mali_userspace::mali_ioctl(fd, version_check_args) != 0) {
                mali_userspace::kbase_ioctl_version_check _version_check_args =
                        {0, 0};
                if (ioctl(fd, KBASE_IOCTL_VERSION_CHECK_CSF,
                          &_version_check_args) < 0) {
                    close(fd);
...

    {
        ...
        if (mali_ioctl(fd, props) == 0) {
            ...
            for (uint32_t i = 0; i < props.props.coherency_info.num_core_groups;
                 i++)
                hw_info.core_mask |=
                        props.props.coherency_info.group[i].core_mask;
...
#define READ_U64(p) (READ_U32((p)) | (uint64_t(READ_U32((p) + 4)) << 32))
            ...
            mali_userspace::gpu_props gprops = {};
            ...
            while (size > 0) {
                ...
                for (unsigned i = 0;
                     mali_userspace::gpu_property_mapping[i].type; i++) {
                    if (mali_userspace::gpu_property_mapping[i].type ==
                        (type >> 2)) {
                        auto offset =
                                mali_userspace::gpu_property_mapping[i].offset;
                        void* p = reinterpret_cast<uint8_t*>(&gprops) + offset;
            ...
            hw_info.gpu_id = gprops.product_id;
            hw_info.r_value = gprops.major_revision;
            hw_info.p_value = gprops.minor_revision;
            for (uint32_t i = 0; i < gprops.num_core_groups; i++)
                hw_info.core_mask |= gprops.core_mask[i];

            // Note: mate40 have bug in core mask, fix it temporaryly
            if (hw_info.gpu_id == 37378) {
                hw_info.core_mask = gprops.core_mask[1];
            }

            hw_info.mp_count = __builtin_popcountll(hw_info.core_mask);
            hw_info.l2_slices = gprops.l2_slices;
...
MaliProfiler::MaliProfiler(const GpuCounterSet& enabled_counters)
    ...
    const std::unordered_map<GpuCounter, MaliValueGetter, GpuCounterHash>
            valhall_csf_mappings = {
                    {GpuCounter::GpuCycles,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_JM,
                                                  "GPU_ACTIVE");
                     }},
                    ...
                    {GpuCounter::FragmentCycles,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_JM,
                                                  "ITER_FRAGMENT_ACTIVE");
                     }},
                    {GpuCounter::TilerCycles,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_TILER,
                                                  "TILER_ACTIVE");
                     }},
                    ...
                    {GpuCounter::CulledPrimitives,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_TILER,
                                                  "PRIM_CULLED") +
                                get_counter_value(MALI_NAME_BLOCK_TILER,
                                                  "PRIM_CLIPPED") +
                                get_counter_value(MALI_NAME_BLOCK_TILER,
                                                  "PRIM_SAT_CULLED");
                     }},
                    ...
                    {GpuCounter::Tiles,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "FRAG_PTILES");
                     }},
                    ...
                    {GpuCounter::Instructions,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "EXEC_INSTR_FMA") +
                                get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "EXEC_INSTR_CVT") +
                                get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "EXEC_INSTR_SFU") +
                                get_counter_value(MALI_NAME_BLOCK_SHADER,
                    ...
                    // The three units run in parallel so we can approximate
                    // cycles by taking the largest value. SFU instructions use
                    // 4 cycles per warp.
                    {GpuCounter::ShaderArithmeticCycles,
                     [this] {
                         return std::max(
                                 get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                   "EXEC_INSTR_FMA"),
                                 std::max(get_counter_value(
                                                  MALI_NAME_BLOCK_SHADER,
                                                  "EXEC_INSTR_CVT"),
                                          4 * get_counter_value(
                                                      MALI_NAME_BLOCK_SHADER,
                                                      "EXEC_INSTR_SFU")));
                    ...
                    {GpuCounter::CacheReadLookups,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_MMU,
                                                  "L2_READ_LOOKUP");
                     }},
    ...
    const std::unordered_map<GpuCounter, MaliValueGetter, GpuCounterHash>
            valhall_mappings = {
                    ...
                    {GpuCounter::ExternalMemoryReadAccesses,  // one access is
                                                              // one cacheline
                                                              // from exteral
                                                              // memory(ddr),
                                                              // usually is 64
                    ...
                    {GpuCounter::ExternalMemoryReadBytes,
                     [this] {
                         // beats is one transfer through AXI data bus, the AXI
                         // data bus in mali gpu is usually 128 bit
                         return get_counter_value(MALI_NAME_BLOCK_MMU,
                                                  "L2_EXT_READ_BEATS") *
                                16;
                     }},
                    ...
                    {GpuCounter::ComputeActive,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "COMPUTE_ACTIVE");
                     }},
                    {GpuCounter::ComputeTasks,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "COMPUTE_TASKS");
                     }},
                    {GpuCounter::ComputeWarps,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "COMPUTE_WARPS");
                     }},
                    {GpuCounter::ComputeStarving,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "COMPUTE_STARVING");
                     }},
                    {GpuCounter::WarpRegSize64,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "WARP_REG_SIZE_64");
                     }},
                    {GpuCounter::LoadStoreReadFull,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "LS_MEM_READ_FULL");
                     }},
                    {GpuCounter::LoadStoreReadPartial,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "LS_MEM_READ_SHORT");
                     }},
                    {GpuCounter::LoadStoreWriteFull,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "LS_MEM_WRITE_FULL");
                     }},
                    {GpuCounter::LoadStoreWritePartial,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "LS_MEM_WRITE_SHORT");
                     }},
                    {GpuCounter::LscRdBeats,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "BEATS_RD_LSC");
                     }},
                    {GpuCounter::LscRdExtBeats,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "BEATS_RD_LSC_EXT");
                     }},
                         ...
                         return std::max(
                                        ...
                                        std::max(
                                                ...
                                                4 * get_counter_value(
                                                            ...
                                                            "EXEC_INSTR_"
                                                            "SFU"))) /
                                (float)(get_counter_value(
                                        MALI_NAME_BLOCK_SHADER,
                                        "EXEC_CORE_ACTIVE"));
                     }},
                    {GpuCounter::LoadStoreUtil,
                     [this] {
                         return (get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                   "LS_MEM_READ_FULL") +
                                 get_counter_value(MALI_NAME_BLOCK_SHADER,
                                 ...
                                 get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                   "LS_MEM_ATOMIC")) /
                                (float)get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                         "EXEC_CORE_ACTIVE");
                     }},
                    {GpuCounter::PartialReadRatio,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "LS_MEM_READ_SHORT") /
                                (float)(get_counter_value(
                    ...
                    {GpuCounter::GBPs,
                     [this] {
                         return (get_counter_value(MALI_NAME_BLOCK_MMU,
                                                   "L2_EXT_READ_BEATS") +
                                 get_counter_value(MALI_NAME_BLOCK_MMU,
                                                   "L2_EXT_WRITE_BEATS")) *
                                16 / std::max<float>(1e-5, get_kern_time());
                     }},
                    {GpuCounter::L2ReadMissRatio,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_MMU,
                                                  "L2_EXT_READ") /
                                (float)get_counter_value(MALI_NAME_BLOCK_MMU,
                                                         "L2_READ_LOOKUP");
                     }},
                    {GpuCounter::L2WriteMissRatio,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_MMU,
                                                  "L2_EXT_WRITE") /
                                (float)get_counter_value(MALI_NAME_BLOCK_MMU,
                    ...
                    {GpuCounter::WarpDivergenceRatio,
                     [this] {
                         return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                                  "EXEC_INSTR_DIVERGED") /
                                (float)(get_counter_value(
    ...
    auto product = std::find_if(std::begin(mali_userspace::products),
                                std::end(mali_userspace::products),
                                [&](const mali_userspace::CounterMapping& cm) {
                                    return (cm.product_mask & gpu_id_) ==
                                           cm.product_id;
                                });
    ...
    if (product != std::end(mali_userspace::products)) {
        switch (product->product_id) {
            ...
            case mali_userspace::PRODUCT_ID_T72X:
                ...
                mappings_[GpuCounter::Pixels] = [this]() {
                    return get_counter_value(MALI_NAME_BLOCK_JM, "JS0_TASKS") *
                           256;
                };
                break;
            ...
            case mali_userspace::PRODUCT_ID_THEX:
                ...
                mappings_[GpuCounter::ShaderTextureCycles] = [this] {
                    return get_counter_value(MALI_NAME_BLOCK_SHADER,
                                             "TEX_COORD_ISSUE");
                };
                break;
        ...
        mperf_throw(MperfError, "failed to identify GPU");
    }
}
...
void MaliProfiler::init() {
    ...
    mperf_log("the num_cores %d, and l2 slices %d\n", num_cores_,
              num_l2_slices_);
...

    {
        ...
        mali_userspace::kbase_uk_hwcnt_reader_version_check_args
                version_check_args;
        version_check_args.header.id =
                mali_userspace::UKP_FUNC_ID_CHECK_VERSION_JM;
        version_check_args.major = 10;
        version_check_args.minor = 2;
        ...
        if (mali_userspace::mali_ioctl(fd_, version_check_args) != 0) {
            mali_userspace::kbase_ioctl_version_check _version_check_args = {0,
                                                                             0};
            if (ioctl(fd_, KBASE_IOCTL_VERSION_CHECK_JM, &_version_check_args) <
                0) {
                checked_version = false;
            }
        }
        ...
        if (!checked_version) {
            ...
            if (mali_userspace::mali_ioctl(fd_, version_check_args) != 0) {
                mali_userspace::kbase_ioctl_version_check _version_check_args =
                        {0, 0};
                if (ioctl(fd_, KBASE_IOCTL_VERSION_CHECK_CSF,
                          &_version_check_args) < 0) {
                    close(fd_);
                    throw std::runtime_error("Failed to check version.");
                }
...

    {
        ...
        setup.header.id =
                mali_userspace::KBASE_FUNC_HWCNT_READER_SETUP;  // NOLINT
        ...
        if (mali_userspace::mali_ioctl(fd_, setup) != 0) {
            ...
            int ret;
            if ((ret = ioctl(fd_, KBASE_IOCTL_HWCNT_READER_SETUP, &_setup)) <
                0) {
...

    {
        ...
        if (ioctl(xpmu_fd_, mali_userspace::KBASE_HWCNT_READER_GET_API_VERSION,
                  &api_version) != 0)  // NOLINT
        {
    ...
    if (ioctl(xpmu_fd_,
              static_cast<int>(
                      mali_userspace::KBASE_HWCNT_READER_GET_BUFFER_SIZE),
              &buffer_size_) != 0)  // NOLINT
    {
        throw std::runtime_error("Failed to get buffer size.");
    }

    if (ioctl(xpmu_fd_,
              static_cast<int>(mali_userspace::KBASE_HWCNT_READER_GET_HWVER),
              &hw_ver_) != 0)  // NOLINT
    {
    ...
    sample_data_ =
            static_cast<uint8_t*>(mmap(nullptr, buffer_count_ * buffer_size_,
                                       PROT_READ, MAP_PRIVATE, xpmu_fd_, 0));
    ...
    auto product = std::find_if(std::begin(mali_userspace::products),
                                std::end(mali_userspace::products),
                                [&](const mali_userspace::CounterMapping& cm) {
                                    return (cm.product_mask & hw_info.gpu_id) ==
                                           cm.product_id;
                                });
...
void MaliProfiler::sample_counters() {
    if (ioctl(xpmu_fd_, mali_userspace::KBASE_HWCNT_READER_DUMP, 0) != 0) {
...
void MaliProfiler::wait_next_event() {
    ...
    if ((poll_fd.revents & POLLIN) != 0) {
        ...
        if (ioctl(xpmu_fd_,
                  static_cast<int>(
                          mali_userspace::KBASE_HWCNT_READER_GET_BUFFER),
                  &meta) != 0)  // NOLINT
        {
            throw std::runtime_error("Failed READER_GET_BUFFER.");
        }
        memcpy(raw_counter_buffer_.data(),
               sample_data_ + buffer_size_ * meta.buffer_idx, buffer_size_);
        timestamp_ = meta.timestamp;
        sample_duration = timestamp_ - timestamp_old_;
        timestamp_old_ = timestamp_;

        if (ioctl(xpmu_fd_, mali_userspace::KBASE_HWCNT_READER_PUT_BUFFER,
                  &meta) != 0)  // NOLINT
        {
...
uint64_t MaliProfiler::get_counter_value(
        mali_userspace::MaliCounterBlockName block, const char* name) const {
    uint64_t sum = 0;
    switch (block) {
        case mali_userspace::MALI_NAME_BLOCK_MMU:
            ...
            for (int i = 0; i < num_l2_slices_; i++) {
                sum += get_counters(block,
                                    i)[find_counter_index_by_name(block, name)];
            }
            return sum;

        case mali_userspace::MALI_NAME_BLOCK_SHADER:
            // If a shader core counter is selected, sum the values over shader
            // cores
        ...
        default:
            return static_cast<uint64_t>(get_counters(
                    block)[find_counter_index_by_name(block, name)]);
    }
}

const uint32_t* MaliProfiler::get_counters(
        mali_userspace::MaliCounterBlockName block, int index) const {
    switch (block) {
        case mali_userspace::MALI_NAME_BLOCK_JM:
            return raw_counter_buffer_.data() +
                   mali_userspace::MALI_NAME_BLOCK_SIZE * 0;
        case mali_userspace::MALI_NAME_BLOCK_MMU:
            ...
            return raw_counter_buffer_.data() +
                   mali_userspace::MALI_NAME_BLOCK_SIZE * (2 + index);
        case mali_userspace::MALI_NAME_BLOCK_TILER:
            return raw_counter_buffer_.data() +
                   mali_userspace::MALI_NAME_BLOCK_SIZE * 1;
        default:
            if (index < 0 || index >= num_cores_) {
                throw std::runtime_error("Invalid core number.");
            }

            // If a shader core counter is selected, index refers to the core
            // index
            return raw_counter_buffer_.data() +
                   mali_userspace::MALI_NAME_BLOCK_SIZE *
                           (2 + num_l2_slices_ + core_index_remap_[index]);
    }
}

int MaliProfiler::find_counter_index_by_name(
        mali_userspace::MaliCounterBlockName block, const char* name) const {
    const char* const* names =
            &names_lut_[mali_userspace::MALI_NAME_BLOCK_SIZE * block];
...
uint64_t MaliProfiler::get_sample_duration() const {
    return sample_duration;
}

uint64_t MaliProfiler::get_kern_time() const {
    if (kern_time == 0) {
...
void MaliProfiler::set_kern_time(uint64_t kern_time_) {
