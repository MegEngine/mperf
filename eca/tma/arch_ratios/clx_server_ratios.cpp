...
static float Mem_L2_Store_Cost = 11;
...
float DRAM_Bound::compute(FEV EV) {
    val = (MEM_Bound_Ratio(EV, 3, thresh) - PMM_Bound::compute(EV));
    val = std::min<float>(val, 1);
    thresh = (val > 0.1) && Memory_Bound::thresh;
    return val;
}
...
float Slow_Pause::compute(FEV EV) {
    val = 40 * g_ev_error("ROB_MISC_EVENTS.PAUSE_INST", 6) /
          CLKS(EV, 6, thresh);
...
float OCR_all_rfo_l3_hit_snoop_hitm(FEV EV, int level, bool& thresh) {
    return g_ev_error("OCR.DEMAND_RFO.L3_HIT.HITM_OTHER_CORE", level) +
           g_ev_error("OCR.PF_L2_RFO.L3_HIT.HITM_OTHER_CORE", level);
}
float ORO_Demand_RFO_C1(FEV EV, int level, bool& thresh) {
    auto func = [&]() -> float {
...
float Mem_Local_DRAM_Cost(FEV EV, int level, bool& thresh) {
    return 80 * Average_Frequency(EV, level, thresh);
}
float Mem_Remote_DRAM_Cost(FEV EV, int level, bool& thresh) {
    return 147.5 * Average_Frequency(EV, level, thresh);
}
float Mem_Remote_HitM_Cost(FEV EV, int level, bool& thresh) {
    return 110 * Average_Frequency(EV, level, thresh);
}
float Mem_Remote_Fwd_Cost(FEV EV, int level, bool& thresh) {
    return 110 * Average_Frequency(EV, level, thresh);
}
...
float Power_License0_Utilization(FEV EV, int level, bool& thresh) {
    if (smt_enabled) {
        return g_ev_error("CORE_POWER.LVL0_TURBO_LICENSE", level) / 2 /
               CORE_CLKS(EV, level, thresh);
    } else {
...
float Power_License1_Utilization(FEV EV, int level, bool& thresh) {
    if (smt_enabled) {
        return val = g_ev_error("CORE_POWER.LVL1_TURBO_LICENSE", level) / 2 /
                     CORE_CLKS(EV, level, thresh);
    } else {
        return g_ev_error("CORE_POWER.LVL1_TURBO_LICENSE", level) /
               CORE_CLKS(EV, level, thresh);
    }
    thresh = (val > 0.5);
    return val;
...
float Power_License2_Utilization(FEV EV, int level, bool& thresh) {
    if (smt_enabled) {
        return val = g_ev_error("CORE_POWER.LVL2_TURBO_LICENSE", level) / 2 /
                     CORE_CLKS(EV, level, thresh);
    } else {
        return g_ev_error("CORE_POWER.LVL2_TURBO_LICENSE", level) /
               CORE_CLKS(EV, level, thresh);
    }
    thresh = (val > 0.5);
    return val;
...
float MEM_PMM_Read_Latency(FEV EV, int level, bool& thresh) {
    return (OneBillion *
            (g_ev_error("UNC_M_PMM_RPQ_OCCUPANCY.ALL", level) /
             g_ev_error("UNC_M_PMM_RPQ_INSERTS", level)) /
            g_ev_error("UNC_M_CLOCKTICKS:one_unit", level));
}
float MEM_DRAM_Read_Latency(FEV EV, int level, bool& thresh) {
    return OneBillion *
           (g_ev_error("UNC_M_RPQ_OCCUPANCY", level) /
            g_ev_error("UNC_M_RPQ_INSERTS", level)) /
           g_ev_error("UNC_M_CLOCKTICKS:one_unit", level);
}
float PMM_Read_BW(FEV EV, int level, bool& thresh) {
...
float IO_Write_BW(FEV EV, int level, bool& thresh) {
    return (g_ev_error("UNC_IIO_DATA_REQ_OF_CPU.MEM_READ.PART0", level) +
            g_ev_error("UNC_IIO_DATA_REQ_OF_CPU.MEM_READ.PART1", level) +
            g_ev_error("UNC_IIO_DATA_REQ_OF_CPU.MEM_READ.PART2", level) +
            g_ev_error("UNC_IIO_DATA_REQ_OF_CPU.MEM_READ.PART3", level)) *
           4 / OneBillion / Time(EV, level, thresh);
}
float IO_Read_BW(FEV EV, int level, bool& thresh) {
    return (g_ev_error("UNC_IIO_DATA_REQ_OF_CPU.MEM_WRITE.PART0", level) +
            g_ev_error("UNC_IIO_DATA_REQ_OF_CPU.MEM_WRITE.PART1", level) +
            g_ev_error("UNC_IIO_DATA_REQ_OF_CPU.MEM_WRITE.PART2", level) +
            g_ev_error("UNC_IIO_DATA_REQ_OF_CPU.MEM_WRITE.PART3", level)) *
           4 / OneBillion / Time(EV, level, thresh);
}
float Time(FEV EV, int level, bool& thresh) {
...
CLXSERVERSetUpImpl::CLXSERVERSetUpImpl() {
