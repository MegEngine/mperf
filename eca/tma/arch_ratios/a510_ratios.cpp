/**
 * \file eca/tma/arch_ratios/a510_ratios.cpp
 *
...
static float Pipeline_Width = 3;

static float OneMillion = 1000000;
static float OneBillion = 1000000000;

static float Cacheline_size = 64;    // unit: bytes
static float ACE_bus_bitwidth = 32;  // unit: bytes

static float LS_rd_bitwidth = 32;  // unit: bytes
...
public:
    Fetch_Bandwidth() {
        ...
        area = "FE";
        level = 2;
        desc = "the value of Fetch_Bandwidth is zero on a510 core";
        parent = nullptr;
        func_compute = (void*)&compute;
    }
    static float compute(FEV EV);
...
// Note: L2D_CACHE_REFILL_PREFETCH on a510 is not count when the soc is
...
float L2D_WR_Miss_Ratio(FEV EV, int level, bool& thresh) {
    return EV({"L2D_CACHE_REFILL_WR", 0x53, 4, 0}, level) /
           EV({"L2D_CACHE_WR", 0x51, 4, 0}, level);
}
float L3D_Miss_Ratio(FEV EV, int level, bool& thresh) {
...
float L3D_WR_Miss_Ratio(FEV EV, int level, bool& thresh) {
    mperf_log_warn(
            "L3D_CACHE_REFILL_WR and L3D_CACHE_WR are not defined in a510 "
            "pmu.");
    return 0;
}
float BR_Mispred_Ratio(FEV EV, int level, bool& thresh) {
...
A510SetUpImpl::A510SetUpImpl() {
    ...
    // Note: Metric_L3D_WR_Miss_Ratio is not support in arm a510
    // m_vmtc_extra.push_back(std::make_pair(
