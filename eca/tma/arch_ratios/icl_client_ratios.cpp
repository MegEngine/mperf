...
public:
    Few_Uops_Instructions() {
        ...
        desc = "This metric represents fraction of slots where the CPU was "
               ...
               "four uops. This highly-correlates with the number of uops in "
               "such instructions.";
        parent = nullptr;
        func_compute = (void*)&compute;
    }
...
public:
    Metric_FP_Arith_Utilization() {
        ...
        desc = "Actual per-core usage of the Floating Point non-X87 execution "
               ...
               "are possible due to Fused-Multiply Add (FMA counting - common; "
               "[ADL+] use all of ADD/MUL/FMA in Scalar or 128/256-bit vectors "
               "- less common).";
        parent = nullptr;
        func_compute = (void*)&compute;
    }
...
float Slow_Pause::compute(FEV EV) {
    val = 140 * g_ev_error("MISC_RETIRED.PAUSE_INST", 6) / CLKS(EV, 6, thresh);
...
float Mem_XSNP_HitM_Cost(FEV EV, int level, bool& thresh) {
    return 32.5 * Average_Frequency(EV, level, thresh);
}
float Mem_XSNP_Hit_Cost(FEV EV, int level, bool& thresh) {
    return 27 * Average_Frequency(EV, level, thresh);
}
float Mem_XSNP_None_Cost(FEV EV, int level, bool& thresh) {
    return 12.5 * Average_Frequency(EV, level, thresh);
}
float Mem_L2_Hit_Cost(FEV EV, int level, bool& thresh) {
...
ICLCLIENTSetUpImpl::ICLCLIENTSetUpImpl() {
