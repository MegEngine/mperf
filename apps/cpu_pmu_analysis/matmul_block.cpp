...
void my_matmul_block(int m, int n, int k, float* a, int lda, float* b, int ldb,
                     float* c, int ldc) {
    int j, p, pb, ib;
    for (p = 0; p < k; p += kc) {
        pb = min(k - p, kc);
...
void AddDot8x12(int k, float* a, int lda, float* b, int ldb, float* c,
    ...
    float
            /* Point to the current elements in the four rows of A */
            *a_0p_pntr,
            *a_1p_pntr, *a_2p_pntr, *a_3p_pntr, *a_4p_pntr, *a_5p_pntr,
            *a_6p_pntr, *a_7p_pntr;
...
void gettma(int m, int n, int k) {
    ...
    // warm up
    my_matmul_block(m, n, k, a, k, b, n, c, n);
