...
for line in f:
  ...
  if 'AI_DRAM' in line:
    linesp = line.split()
    linesp = linesp[1:]
    AI_DRAM = [float(a) for a in linesp]
    print('AI_DRAM', AI_DRAM)
  if 'AI_L3' in line:
    linesp = line.split()
    linesp = linesp[1:]
    AI_L3 = [float(a) for a in linesp]
    print('AI_L3', AI_L3)
  if 'AI_L2' in line:
    linesp = line.split()
    linesp = linesp[1:]
...
for i in range(0,len(AI_L2)):
  ax.plot(float(AI_L2[i]),float(FLOPS[0]),c=colors[0],marker=styles[i],linestyle='None',ms=markersize,label=labels[i])
#ax.plot((AI_L2),(FLOPS),c=colors[0],linestyle='-')
for i in range(0,len(AI_L3)):
  ax.plot(float(AI_L3[i]),float(FLOPS[1]),c=colors[1],marker=styles[i],linestyle='None',ms=markersize,label=labels[i])
#ax.plot((AI_L3),(FLOPS),c=colors[1],linestyle='-')
for i in range(0,len(AI_DRAM)):
  ax.plot(float(AI_DRAM[i]),float(FLOPS[2]),c=colors[2],marker=styles[i],linestyle='None',ms=markersize,label=labels[i])
  marker_handles.append(ax.plot([],[],c='gray',marker=styles[i],linestyle='None',ms=markersize,label=labels[i])[0]) 
#ax.plot((AI_DRAM),(FLOPS),c=colors[2],linestyle='-')
